...
guideUsers.forEach((user, index) => {
  const guideCities = faker.helpers.arrayElements(db.cities, {
    min: 1,
    max: 3,
  });

  // --- NOUVELLE LOGIQUE DE DISPONIBILITÉ ---
  const daysOfWeek = [
    "Monday",
    "Tuesday",
    "Wednesday",
  ...
  const availability = {
    // Statut global (Disponible maintenant ou pas)
    isCurrentlyAvailable: faker.datatype.boolean(0.7),
    // Jours travaillés et horaires
    schedule: daysOfWeek.map((day) => ({
      day: day,
      // La plupart des guides ne travaillent pas le dimanche (exemple)
      isOpen:
        day === "Sunday"
      ...
      slots: [
        {
          start: "09:00",
          end: faker.helpers.arrayElement(["12:00", "13:00", "17:00", "18:00"]),
        },
      ],
    })),
    // Dates spécifiques où le guide est absent (vacances, etc.)
    unavailableDates: [faker.date.future().toISOString().split("T")[0]],
  };
  // ------------------------------------------
  ...
  db.guideProfiles.push({
    ...
    specialties: faker.helpers.arrayElements(
      ["Architecture", "Food", "History", "Nightlife", "Shopping"],
      3,
    ),
...
for (let i = 1; i <= COUNTS.places; i++) {
  ...
  const guideOwner = faker.datatype.boolean()
    ? faker.helpers.arrayElement(guideUsers)
    : null;
  ...
  db.places.push({
    ...
    name:
      faker.company.name() +
      " " +
      faker.helpers.arrayElement(["Museum", "Restaurant", "Park", "Monument"]),
    slug: `place-${i}`,
    description: faker.lorem.paragraph(),
    ...
    owner: guideOwner
      ? {
          id: guideOwner.id,
          firstName: guideOwner.firstName,
          lastName: guideOwner.lastName,
          avatarUrl: guideOwner.avatarUrl,
          role: "guide",
        }
      : null,
...
const generateEngagement = (targetType, target) => {
  ...
  for (let j = 0; j < numScores; j++) {
    db.scores.push({
      ...
      authorId: faker.helpers.arrayElement(db.users).id,
  ...
  for (let j = 0; j < numMedia; j++) {
    db.media.push({
      id: `m${mediaIdCounter++}`,
      url: `https://picsum.photos/seed/media${mediaIdCounter}/800/500`,
      type: "image",
  ...
  for (let j = 0; j < numComments; j++) {
    ...
    const parentId = isReply
      ? faker.helpers.arrayElement(parentComments)
      : null;

    const comment = {
      id: `com${commentIdCounter++}`,
      targetId: target.id,
      targetType: targetType,
      authorId: author.id,
      author: {
